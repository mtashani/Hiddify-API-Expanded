...
@contextmanager
def redis_lock(lock_key="lock-update-local-usage", timeout=60, sleep=0.1):
    
    acquired = False
    try:
        while not acquired:
            acquired = cache.redis_client.set(lock_key, "locked", nx=True, ex=timeout)
            if not acquired:
                time.sleep(sleep)
        yield  
    finally:
...
def update_local_usage():
    try:
        with redis_lock("lock-update-local-usage", timeout=60):
